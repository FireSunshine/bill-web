...
const PopupAddBill = forwardRef(({ detail = {}, onReload }, ref) => {
  ...
  const [showRemark, setShowRemark] = useState(); // 备注输入框展示控制
  ...
  const addBill = async () => {
    ...
    const params = {
      ...
      date: dayjs(date).unix() * 1000, // 时间传时间戳
      ...
      <div className={s.addWrap}>
        ...
        <div className={s.typeWarp}>
          <div className={s.typeBody}>
            ...
            {
              (payType === 'expense' ? expense : income).map(item => <div key={item.id} className={s.typeItem} onClick={() => setCurrentType(item)}>
